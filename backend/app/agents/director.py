...
async def director_node(state: DirectorState, config: dict) -> dict:
    ...
    # For demo UX: first turn must produce at least one answer.
    # If model returns END on the first turn, force qa fallback.
    if agent_id is None and cleaned == "end":
        if not agent_responses:
            logger.warning("Director: first-turn END detected — forcing 'qa' fallback")
