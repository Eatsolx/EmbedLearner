...
def _get_knowledge_service(llm):
    class _FallbackKnowledgeService:
        async def search(self, q: str, course_id: str, top_k: int = 3):
            return []

        async def get_graph(self, course_id: str | None = None):
            return {"nodes": [], "edges": []}

        async def get_cross_course(self, point_id: str):
            return []
    ...
    try:
        ...
        logger.warning("KnowledgeService unavailable, falling back to empty service: %s", e)
        ...
        async def event_generator():
            ...
            async def run_graph():
                ...
                try:
                    ...
                    logger.exception("Agent graph execution failed; switching to direct LLM fallback")
                    try:
                        llm = _get_llm_client()
                        await events_queue.put(
                            AgentStart(agent_id="qa", agent_name="智能答疑 Agent")
                        )
                        async for chunk in llm.stream(
                            ...
                            await events_queue.put(TextDelta(content=chunk))
                        await events_queue.put(AgentEnd(agent_id="qa"))
                        agent_count = 1
                    except Exception as fallback_error:
                        logger.exception("Direct LLM fallback failed")
                        await events_queue.put(
                            Error(message=f"LLM 调用失败: {fallback_error}")
                        )
                finally:
