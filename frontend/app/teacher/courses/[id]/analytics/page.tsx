    ...
    >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-ink-border bg-white p-5">
          ...
          {data.mastery_distribution.some((item) => item.count > 0) ? (
            ...
            <div className="flex h-[280px] items-center justify-center rounded-lg border border-dashed border-ink-border bg-ink-surface/40 text-center">
              <p className="text-xs text-ink-text-muted">
                暂无分布数据，学生产生学习行为后会自动更新。
              </p>
            </div>
          )}
        </div>
          ...
          <div className="space-y-3">
            {data.top_errors.length > 0 ? (
              ...
              <div className="rounded-lg border border-dashed border-ink-border bg-ink-surface/40 p-6 text-center">
                <p className="text-xs text-ink-text-muted">暂无易错知识点数据。</p>
              </div>
            )}
          </div>
        </div>
