...
type WarningRow = WarningStudent & { course_id?: string };
      ...
      <div className="rounded-xl border border-ink-border bg-white overflow-x-auto">
        {sorted.length > 0 ? (
              ...
              {sorted.map((student) => {
                ...
                return (
                  ...
                  <td className="px-5 py-3 text-right">
                    <button
                      onClick={() => router.push(`/teacher/courses/${student.course_id || "00000000-0000-4000-b000-000000000001"}/analytics`)}
          ...
          <div className="p-8 text-center">
            <div className="mx-auto flex h-11 w-11 items-center justify-center rounded-xl bg-ink-success-light text-ink-success">
              <i className="ri-shield-check-line text-xl" />
            </div>
            <p className="mt-3 text-sm font-medium text-ink-text">当前没有预警学生</p>
            <p className="mt-1 text-xs text-ink-text-muted">
              系统会在识别到风险趋势后自动生成预警记录。
            </p>
          </div>
        )}
      </div>
