...
export default function PopupPage() {
  ...
  const courseId = searchParams.get("course_id") || "ai-101";
  const courseName = COURSE_NAMES[courseId] || courseId;

  return (
    <div className="h-full w-full bg-transparent pointer-events-none">
      {/* Floating chat panel */}
      {open && (
        <div className="pointer-events-auto fixed bottom-20 right-6 z-50 flex h-[560px] w-[420px] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border border-ink-border bg-white shadow-xl">
          {/* Header */}
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        className="pointer-events-auto fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-ink-primary text-white shadow-lg transition-all hover:bg-ink-primary-dark hover:shadow-xl active:scale-95"
      >
        <i
